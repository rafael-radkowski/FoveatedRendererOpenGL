...
void CameraControls::cursorCallback( GLFWwindow *window, double x, double y )
{
	...
	if (_mouse_move_event == 1) {
		...
		// zoom in and out to the center point. 
		 ...
		 if(std::abs(distance) > 0.001){
			...
			_eye = eye +  glm::vec3( glm::normalize(dof)) * sign * _locomotion_speed;
	...
	if (_mouse_middle_event == 1) {
		...
		_eye = eye - glm::normalize( glm::vec3(side)) * signx * _locomotion_speed+  glm::normalize( glm::vec3(up)) * signy * _locomotion_speed;
...
void CameraControls::setLocomotionSpeed(float speed)
{
	_locomotion_speed = std::max(0.0f, speed);
}
