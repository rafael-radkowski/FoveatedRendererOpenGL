...
using namespace cs557_OBJModelExt;
...
*/
void cs557::OBJModelExt::create(string path_and_filename, int shader_program)
{
	std::cout << "Loading " << path_and_filename << std::endl;
	program = -1;
	_N = 0;
	_I = 0;
	...
	for(int i=0; i< size; i++)
	{
		...
		if( i == 0 )
		{
			prop.mat.diffuse_mat.x =  curMesh.MeshMaterial.Kd.X;
			prop.mat.diffuse_mat.y =  curMesh.MeshMaterial.Kd.Y;
			prop.mat.diffuse_mat.z =  curMesh.MeshMaterial.Kd.Z;
			prop.mat.specular_mat.x =  curMesh.MeshMaterial.Ks.X;
			prop.mat.specular_mat.y =  curMesh.MeshMaterial.Ks.Y;
			prop.mat.specular_mat.z =  curMesh.MeshMaterial.Ks.Z;
			prop.mat.specular_s =  curMesh.MeshMaterial.Ns;
			prop.mat.ambient_mat.x =  curMesh.MeshMaterial.Ka.X;
			prop.mat.ambient_mat.y =  curMesh.MeshMaterial.Ka.Y;
			prop.mat.ambient_mat.z =  curMesh.MeshMaterial.Ka.Z;
	...
	// assign all materials to the shader program
	prop.mat.setAllUniform(program);
...
*/
void cs557::OBJModelExt::draw(glm::mat4 projectionMatrix, glm::mat4 viewMatrix)
{
	...
	glUniformMatrix4fv(modelMatrixLocation, 1, GL_FALSE, &prop.model_matrix[0][0]); // send the model matrix to our shader
