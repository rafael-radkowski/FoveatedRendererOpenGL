...
@param texture_depth -  a variable in which this function can write the depth texture idx into.
*/
inline void CreateRenderBuffer(int texture_width, int texture_height, unsigned int& frame_buffer_object, unsigned int& texture_color,  unsigned int& texture_depth)
{
	// Create a frame buffer
	//glCreateFramebuffers(1, &frame_buffer_object);
	glGenFramebuffers(1, &frame_buffer_object);
	...
	unsigned int rbo;
	glGenRenderbuffers(1, &texture_color);
	glBindRenderbuffer(GL_RENDERBUFFER, texture_color); 
	glRenderbufferStorage(GL_RENDERBUFFER, GL_RGB8, texture_width, texture_height); 

	glGenRenderbuffers(1, &texture_depth);
	glBindRenderbuffer(GL_RENDERBUFFER, texture_depth); 
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, texture_width, texture_height); 

	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, texture_color);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, texture_depth);
